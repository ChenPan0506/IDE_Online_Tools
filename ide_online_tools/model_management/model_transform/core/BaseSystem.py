...
from ide_online_tools.model_management.model_transform.core.BaseSheet import BaseSheet
from ide_online_tools.model_management.model_transform.core.BasePart import BasePart
                                ...
                                if pd.isna(self.sysdatatable[0][element[0]][j]) or self.sysdatatable[0][element[0]][
                                    ...
                                    if pd.isna(content) or content == "[,]":
                                        ...
                                        if content1[1] == "1" and content2[1] == "1":
                                            ...
                                            result_list[1] = "X"
                                        self.sysdatatable[0][element[0]][j] = str(result_list).replace("'", "")
        ...
        for col_number in sum_fm_col.columns.tolist():
            if col_number == 2:
                ...
                self.sysdatatable[0].iloc[2, col_number] = temp_fm["f"][
                    (temp_fm["sum_f_FM"] == "[X,1]") | (temp_fm["sum_f_FM"] == "[X,0]") | (temp_fm["sum_f_FM"] == "[X,X]")].sum()
                self.sysdatatable[0].iloc[self.sys_full_cursor["IS"][0], col_number] = temp_io["sum_f_IO"][
                    (temp_io["sum_f_IO"] == "[X,1]") | (temp_io["sum_f_IO"] == "[X,0]") | (temp_io["sum_f_IO"] == "[X,X]")].count()
