...
class TConfigModelLink(db.Model):
    ...
    create_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP"), info='创建时间')
...
class TDtcInfo(db.Model):
    ...
    table_id = db.Column(db.Integer, primary_key=True, info='表ID')
    dtc_id = db.Column(db.String(22), nullable=False, index=True, info='DTC  ID')
...
class TUserOperationLog(db.Model):
    ...
    save_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"), info='保存时间')
...
class TVehicleConfig(db.Model):
    ...
    modify_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"), info='修改时间')
    create_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP"), info='创建时间')
...
class TVehicleModel(db.Model):
    ...
    vehicle_type_id = db.Column(db.String(22), nullable=False, index=True, info='车型编号')
    vehicle_config_id = db.Column(db.String(22), nullable=False, index=True, info='配置编号')
    # vehicle_type_id = db.Column(db.ForeignKey('t_vehicle_type.vehicle_type_id', ondelete='CASCADE', onupdate='CASCADE'),
    ...
    release_time = db.Column(db.DateTime, nullable=False, info='发布时间')
    import_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP"), info='导入时间')
    modify_time = db.Column(db.DateTime, server_default=text("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"), info='修改时间')
